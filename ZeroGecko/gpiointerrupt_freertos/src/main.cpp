...
void gpioCallback(uint8_t pin) {
  switch (pin) {
    case 8: {
      xSemaphoreGiveFromISR(parametersToTask1.semaphor, NULL);
      break;
    }
    case 9: {
      xSemaphoreGiveFromISR(parametersToTask0.semaphor, NULL);
      break;
    }
  }
}
...
void taskLed0(void *pParameters) {
  ...
  for (;;) {
    if (xSemaphoreTake(pData->semaphor, 0) == pdTRUE) {
...
int main(void) {
  ...
  parametersToTask0.semaphor = xSemaphoreCreateBinary();

  parametersToTask1.port = gpioPortC;
  parametersToTask1.pin = 11;
  parametersToTask1.semaphor = xSemaphoreCreateBinary();
